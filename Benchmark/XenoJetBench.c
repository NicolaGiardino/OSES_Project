...
{
  float deriv, machn, macho, airo, airn, number, chokair; // iterate for mach number
  int iter;
...
{
    float number, wc1, wc2, mgueso, mach2, g1, gm1, g2, gm2, fac1, fac2, fac3, fac4;
    g1 = getGama(tlow);
    gm1 = g1 - 1.0;
    wc1 = getAir(mach1, g1);
...
void calcPerf()
{
    ...
    if (engine == 1)
    {
        weight = .12754 * sqroot(acore * acore * acore) * (dcomp * lcomp + dburner * lburn + dturbin * lturb + dnozl * lnoz);
    }
    if (engine == 2)
    {
        weight = .08533 * sqroot(acore * acore * acore) * (dcomp * lcomp + dburner * lburn + dturbin * lturb + dnozl * lnoz);
    }
    if (engine == 3)
    {
