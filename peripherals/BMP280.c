...
#define BMP280_REG_CTRL_MEAS                                                   \
  ((uint8_t)0xF4) // Pressure and temperature measure control register
...
static BMP280_RESULT __reg_read_bulk(rt_uint8_t reg, rt_uint8_t *buf,
                                     rt_uint8_t count) {
...
// return: version of BMP280 chip or zero in case of chip absence or error on
// I2C bus
inline rt_uint8_t BMP280_GetVersion(void) { return __reg_read(BMP280_REG_ID); }

// Get current status of the BMP280 chip
// return: status of the chip or zero in case of chip absence or error on I2C
// bus
...
// return: working mode of the chip or zero in case of chip absence or error on
// I2C bus
...
void BMP280_SetMode(rt_uint8_t mode) {
  ...
  rt_uint8_t reg =
      (rt_uint8_t)(__reg_read(BMP280_REG_CTRL_MEAS) & ~BMP280_MODE_MSK);
...
void BMP280_SetFilter(rt_uint8_t filter) {
  ...
  rt_uint8_t reg =
      (rt_uint8_t)(__reg_read(BMP280_REG_CONFIG) & ~BMP280_FILTER_MSK);
...
void BMP280_SetStandby(rt_uint8_t tsb) {
  ...
  tsb &= BMP280_STBY_MSK;
  rt_uint8_t reg =
      (rt_uint8_t)(__reg_read(BMP280_REG_CONFIG) & ~BMP280_STBY_MSK);
...
void BMP280_SetOSRST(rt_uint8_t osrs) {
  ...
  rt_uint8_t reg =
      (rt_uint8_t)(__reg_read(BMP280_REG_CTRL_MEAS) & ~BMP280_OSRS_T_MSK);
...
void BMP280_SetOSRSP(rt_uint8_t osrs) {
  ...
  rt_uint8_t reg =
      (rt_uint8_t)(__reg_read(BMP280_REG_CTRL_MEAS) & ~BMP280_OSRS_P_MSK);
  __reg_write(BMP280_REG_CTRL_MEAS, reg | osrs);
}
...
BMP280_RESULT BMP280_Read_Calibration(void) {
  ...
  return __reg_read_bulk(BMP280_REG_CALIB00, (rt_uint8_t *)&cal_param,
                         sizeof(cal_param));
}
...
BMP280_RESULT BMP280_Read_UT(rt_int32_t *UT) {
  ...
  if (__reg_read_bulk(BMP280_REG_TEMP_MSB, buf, sizeof(buf)) ==
      BMP280_SUCCESS) {
...
BMP280_RESULT BMP280_Read_UP(rt_int32_t *UP) {
  ...
  if (__reg_read_bulk(BMP280_REG_PRESS_MSB, buf, sizeof(buf)) ==
      BMP280_SUCCESS) {
...
rt_int32_t BMP280_CalcT(rt_int32_t UT) {
  ...
  t_fine = ((((UT >> 3) - ((int32_t)cal_param.dig_T1 << 1))) *
            ((int32_t)cal_param.dig_T2)) >>
           11;
  t_fine += (((((UT >> 4) - ((int32_t)cal_param.dig_T1)) *
               ((UT >> 4) - ((int32_t)cal_param.dig_T1))) >>
              12) *
             ((int32_t)cal_param.dig_T3)) >>
            14;
...
// return: pressure, "micro-meters" of mercury (value of '739503' represents
// '739.503mmHg')
rt_uint32_t BMP280_Pa_to_mmHg(rt_uint32_t p_pa) {
  ...
  // (XXX * 61039) / 1000000 is rough integer equivalent of float (XXX /
  // 16383.0F) * 1000
...
// note: BMP280_CalcT of BMP280_CalcTf must be called before calling this
// function note: code from the BMP280 datasheet (rev 1.19)
...
float BMP280_Pa_to_mmHgf(float p_pa) { return p_pa / 133.322368F; }
