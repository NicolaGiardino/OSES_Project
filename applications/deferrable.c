 ...
 * It sets back the priority to its initial one, and restarts it if it
 * self-suspended
 *
...
void rt_timer_deferrable_period(void *param)
{
	...
	rt_thread_control(&deferrable_server.deferrable,
					  RT_THREAD_CTRL_CHANGE_PRIORITY,
					  &deferrable_server.priority);

	rt_timer_control(&deferrable_server.tim_capacity, RT_TIMER_CTRL_SET_TIME,
					 &deferrable_server.capacity_left);
}

/**
 * This function will sort of stop the deferrable server when its capacity is
 * finished The server will be taken to the lowest priority in the system (just
 * above idle)
 *
 * @param NULL parameter passed to the timer function
 *
...
static void rt_thread_deferrable_entry(void *param)
{
	...
	while (1)
	{
		...
		task = rt_list_entry(deferrable_server.asyncTask.next,
							 struct rt_aperiodic_task, tlist);
...
/**
 * This function instantiate all the objects needed to the operation of the
 * server
 *
 ...
 * @return void
 */
rt_err_t rt_thread_deferrable_init(rt_uint32_t capacity, rt_uint32_t period,
								   rt_uint8_t priority)
{
	rt_err_t err;
	...
	rt_thread_init(&deferrable_server.deferrable, deferr_name,
				   rt_thread_deferrable_entry, RT_NULL, rt_deferrable_stack,
				   sizeof(rt_deferrable_stack), priority, capacity);

	rt_sprintf(deferr_name, "Timer");

	rt_timer_init(&deferrable_server.tim_server, deferr_name,
				  rt_timer_deferrable_period, RT_NULL, period,
				  RT_TIMER_FLAG_PERIODIC);

	rt_sprintf(deferr_name, "Capac");

	rt_timer_init(&deferrable_server.tim_capacity, deferr_name,
				  rt_timer_deferrable_capacity, RT_NULL, capacity,
...
{
	/*
	 * If no asynchronous tasks are present in the server, add the first and set
	 * server in rdy state
	 */
	if (rt_list_isempty(deferrable_server.asyncTask.next))
	{
	...
	/*
	 * If asynchronous tasks are present, search for the first position available
	 * in the queue and put the task there
	 */
	else
	{
		rt_list_init(&task->tlist);
