...
int main(void)
{
    ...
    if(msd_init("SDCARD", "spi1"))
    {
        rt_kprintf("Error attaching sdcard\n");
        return;
    }

    rt_device_t dev = RT_NULL;

    dev = rt_device_find("SDCARD");
    ...
    while (1)
    {
        ...
        rt_sprintf(buf, "Acc. X = %d, Y = %d, Z = %d\n", acceleration[0], acceleration[1], acceleration[2]);
        if (rt_device_write(dev, 0, buf, sizeof(buf)) < 0)
        {
            rt_kprintf("f_printf failed\n");
        }
        rt_kprintf("Gyro. X = %d, Y = %d, Z = %d\n", gyro[0], gyro[1], gyro[2]);
        rt_sprintf(buf, "Gyro. X = %d, Y = %d, Z = %d\n", gyro[0], gyro[1], gyro[2]);
        if (rt_device_write(dev, 0, buf, sizeof(buf)) < 0)
        {
            rt_kprintf("f_printf failed\n");
        }
